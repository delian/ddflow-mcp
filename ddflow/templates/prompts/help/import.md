...
believes it, because the tool said so. That is worse than having no tool.
    ...
    ddflow import              look; writes nothing
    ddflow import --apply      write it, each item recording its source line
    ddflow import --verify     is it still true, and did anyone finish it?
...
`ddflow import --verify` answers three things: what is imported and when; whether the
