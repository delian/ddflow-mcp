...
def in_container() -> bool:
    ...
    `DDFLOW_IN_CONTAINER` is set by our own image and is the authoritative signal;
    the rest are the standard fallbacks for an image someone else built.
    """
    if os.environ.get("DDFLOW_IN_CONTAINER") == "1":
        return True
    ...
    for name, url in reviewer_urls or []:
        if any(n in url for n in ("127.0.0.1", "localhost", "[::1]")):
            out.append(
                ...
                f"CONTAINER's loopback, not your machine. ddflow rewrites it to "
