...
Every command below is `orchard ...`. If your harness exposes Orchard over MCP, the
...
```sh
orchard doctor                                    # [orchard_doctor]
orchard recover                                   # [orchard_recover]
SESSION=$(orchard --json session start --model "<your model id>" --tool "<your harness>" | jq -r .session)
orchard session prompt "$SESSION" --text "<the operator's message, verbatim>"
orchard brief --phase <NAME>                      # [orchard_brief]
```

`orchard brief` replaces reading the project's rule files and lesson corpus. It returns
...
named tree, salvage what is real, then `orchard release <id>`. Never delete first.
...
```sh
orchard research --question "<what you needed to know>" \
...
- `CONFIRMED` and `REFUTED` **require** a probe; Orchard refuses them without one.
...
```sh
orchard next --phase <NAME>                       # [orchard_next]
```
...
```sh
orchard claim <ID> --globs "<paths this task will write>"   # [orchard_claim]
```
...
Renew during long work: `orchard heartbeat <ID>` (interval: `lease.heartbeat_s`).

If your task needs to write outside its declared globs, run
`orchard update <ID> --globs "..."` **first**, so the conflict detector can see it.

### 2c. Run the task pipeline

```sh
orchard gate status <ID>                          # [orchard_gate_status]
```

It prints the pipeline, the next gate, and that gate's instruction. The default order is
the ten steps below; `.orchard/gates.toml` changes it per project.

| # | Gate | Who runs it | How to record |
|---|---|---|---|
| 1 | `research` | you | `orchard research ...` then `gate record` |
| 2 | `rules` | you | `orchard brief --item <ID>` |
...
| 6 | `standards` | tooling | `orchard gate run <ID> standards` |
| 7 | `unit_tests` | tooling | `orchard gate run <ID> unit_tests` |
| 8 | `bug_hunt` | you | `gate record` + a probe per finding |
| 9 | `dedupe` | you | `gate record` |
| 10 | `merge` | Orchard | `orchard merge <ID>` |
...
3. **At least one reviewer must be a different pretraining family than you.** Same-family
   agreement is not independent evidence — it measures shared priors. `orchard complete`
   refuses without it. Pass `--model` on every review so Orchard can tell.
...
orchard merge <ID>                                # [orchard_merge]
orchard complete <ID> --model "<your model>" --sha "<sha>"
orchard release <ID>
```
...
```sh
orchard lesson add --title "<the rule, as one line>" --rule "..." --why "..." --how "..."
orchard bug fixed <BUG> --regression-test "<test that now guards this>"
```
...
```sh
orchard cadence                                   # [orchard_cadence]
```
...
When `orchard next --phase <NAME>` reports no remaining tasks:

```sh
orchard gate run <NAME> unit_tests                # the WHOLE suite, not the task's slice
orchard gate record <NAME> bug_hunt   --outcome passed --evidence "..."
orchard gate record <NAME> dedupe     --outcome passed --evidence "..."
orchard gate record <NAME> live_test  --outcome passed --evidence "<real run output>"
orchard gate record <NAME> corrections --outcome passed --evidence "..."
orchard complete <NAME> --model "<your model>"
orchard render                                    # regenerate the human-readable board
```
...
```sh
orchard session end "$SESSION" --summary "<what shipped>"
orchard render && orchard doctor
git add .orchard/events docs/orchard && git commit -m "orchard: session log"
```

**Commit the event log.** It is the source of truth and the only artefact from which the
project can be reconstructed. The index (`.orchard/index.db`) is gitignored and
...
- **Prefer `orchard brief` over reading the rule and lesson files.** That is what it is
