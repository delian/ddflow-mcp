...
Every agent works in its own git worktree, and the queue is what stops them colliding.
    ...
    orchard claim <id>              lease + worktree; exit 3 = refused, with the reason
    orchard claim <id> --no-worktree   for work that edits nothing
...
`orchard help recovery` — nothing is stolen, and nothing is thrown away.
