...
its pipeline carries one, and `orchard complete` names the ones that do not.

Two kinds, and the difference is the whole design:

- **Command gates** run something. `orchard gate run <id> unit_tests` executes the
...
- **Agent gates** are performed by you: a rubber-duck read, a standards check, a bug
  hunt. `orchard gate record <id> <gate> --outcome passed --evidence "..."` — and the
  evidence is the point, because nothing else can check that you did it.

`orchard gate status <id>` shows the pipeline, where the item is in it, and the
...
independent evidence. `orchard reviewers detect` finds one; `orchard review <id> --gate
