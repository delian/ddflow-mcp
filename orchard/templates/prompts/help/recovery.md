# When something goes wrong
    ...
    orchard doctor       integrity and health; exit 1 = problems, each named
    orchard recover      find crashed agents' work; exit 2 = nothing to recover
    orchard rebuild      re-derive the search index from the log
    orchard replay       reconstruct the project's decisions from the log alone

## A crashed agent

Its lease expires; its worktree is still on disk with its work in it. `orchard recover`
...
`orchard recover --apply` adopts it, keeping the work. Nothing is deleted.

## A broken index

The index is a disposable, gitignored SQLite projection. Anything wrong with it is fixed
by `orchard rebuild`, which drops it and re-derives it from the log. There is
...
`orchard replay --out ./recovery-kit` writes a reconstruction addressed to a fresh
