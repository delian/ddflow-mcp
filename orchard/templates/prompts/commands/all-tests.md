...
## 2. Run every kind of test, not just the fast one
    ...
    orchard_gate_run <item> unit_tests
{% for gate in test_gates %}
    orchard_gate_run <item> {{ gate }}
{% endfor %}

Configure the ones this project has in `.orchard/config.toml` — integration, UI, end-to-end, property, smoke, whatever exists:
...
**A suite that is not configured is not passing — it is absent.** `orchard_gate_status` shows `[?]` for a gate that could not run, and that is a coverage gap, never a tick.
...
## 5. Report honestly
    ...
    orchard_gate_record <item> unit_tests --outcome passed --evidence "<command, counts, duration>"
...
## 6. Close out
    ...
    orchard_loops       — a test that keeps flapping is a loop; re-running it will not converge
    orchard_progress    — where the effort went
