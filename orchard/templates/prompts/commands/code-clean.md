...
## 1. Find what was left behind
    ...
    orchard_recover     — crashed agents' worktrees, with what is actually in each
    orchard_cleanup     — every Orchard worktree and branch, classified

`orchard_cleanup` classifies each tree as **merged** (safe to remove), **unmerged** (carries commits nobody landed), **dirty** (uncommitted edits — a human looks), **orphan** (no item claims it), or **stale_branch**.
...
Then either finish it (commit, gate, merge) or release it with a note saying what you concluded. `orchard_release` records that note, and six months from now it is the only explanation of why a claim was broken.
...
For each tree with commits and a finished item: merge it. For each with commits and an *unfinished* item: finish the item properly — gates and all — or abandon it deliberately.
    ...
    orchard_merge <item>
    orchard_complete <item> --model "<your model>"
    orchard_abandon <item> --reason "..."    # if it will not be finished
...
## 4. Remove what is finished
    ...
    orchard_cleanup with apply=true   — removes merged worktrees and merged branches only
...
## 6. Commit, and check nothing is looping
    ...
    orchard_loops       — a clean-up that keeps finding the same work is a loop, not a chore
    orchard_doctor      — integrity, cycles, unknown dependencies, stale index
    orchard_progress    — what the effort actually went into
