...
1. **Iteration.** Copilot's coding agent works one issue/PR at a time. Map one Orchard
   task to one PR. Put `orchard claim <ID>` in the setup step and
   `orchard complete <ID>` in the finish step.
2. **Subagents.** None; parallelism comes from multiple concurrent agent runs, which
   Orchard's leases already coordinate.
