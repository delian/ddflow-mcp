...
Every one of them was correct while those files sat directly under `orchard/`, and every
...
*behaviour*. Anchoring on the package object instead does not: `orchard.__file__` is
...
def package_dir() -> Path:
    """The `orchard/` package directory itself."""
    import orchard

    return Path(orchard.__file__).resolve().parent


@lru_cache(maxsize=1)
def package_parent() -> Path:
    """The directory that must be on `PYTHONPATH` to `import orchard`.
