...
— on the project that motivated Orchard, a killed agent's worktree was repeatedly found
...
def _measure(rec: Recovery, repo: Path, cfg: Config) -> None:
    ...
    if not wt or not wt.exists():
        rec.advice = (
            ...
            f"`orchard lease release {rec.item}` to clear the claim."
        )
        return
    ...
    if rec.salvageable:
        rec.advice = (
            ...
            f"then `orchard lease release {rec.item} --note salvaged`."
        )
    else:
        rec.advice = (
            f"clean and fully merged — safe to remove: "
            f"`orchard worktree remove {rec.item}` "
            f"then `orchard lease release {rec.item}`."
        )
